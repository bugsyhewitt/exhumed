...
func scanEntry(ctx context.Context, eng *engine.Engine, tmpl engine.Request, f scanFlags, entry db.CompiledEntry) (*hitRecord, int) {
	...
	for i, r := range results {
		...
		// --match-size is active and this body length is NOT in the allowlist (the
		// three positive keep-gates, applied first as a conjunction), --filter-size
		// flags this body length as known noise, --filter-code flags this status
		// code as known noise, --filter-regex matches this body's content as known
		// noise, --filter-time flags this round-trip duration as known noise, or
		...
		if !f.onlyHits && f.matchFilter.Keep(r.Body) && f.codeMatcher.Keep(r.StatusCode) && f.sizeMatcher.Keep(len(r.Body)) && !f.sizeFilter.Match(len(r.Body)) && !f.codeFilter.Match(r.StatusCode) && !f.bodyFilter.Match(r.Body) && !f.timeFilter.Match(r.Elapsed) && !f.wordFilter.MatchBody(r.Body) {
